...
namespace wabby::render::vulkan
{
  ...
  std::optional<uint32_t> vk_hardware::queue_index( QueueType type, const std::optional<std::reference_wrapper<const vk::raii::SurfaceKHR>> surface ) const
  {
    ...
    switch ( type )
    {
      case QueueType::PRESENT: index = get_present_queue_index( physical_device_, surface->get() ); break;
      case QueueType::GRAPHICS: index = get_first_queue_index( physical_device_, vk::QueueFlagBits::eGraphics ); break;
                                                       ...
                                                       const std::vector<std::string_view> & desired_extensions )
  {
    ...
    std::transform(
      extension_props.begin(), extension_props.end(), std::inserter( extensions, extensions.end() ), []( auto && prop ) { return prop.extensionName.data(); } );
  ...
  vk::raii::Device build_device( const vk::raii::PhysicalDevice & physical_device )
  {
    ...
    std::transform(
      required_extensions.begin(), required_extensions.end(), std::back_inserter( enable_extensions ), []( auto && ext ) { return ext.c_str(); } );
    std::transform(
      optional_extensions.begin(), optional_extensions.end(), std::back_inserter( enable_extensions ), []( auto && ext ) { return ext.c_str(); } );
    ...
    for ( uint32_t i = 0; i < queue_families.size(); i++ )
    {
      device_queue_create_infos.push_back(
        vk::DeviceQueueCreateInfo{ .queueFamilyIndex = i, .queueCount = 1, .pQueuePriorities = &device_queue_priorities[i] } );
    }
