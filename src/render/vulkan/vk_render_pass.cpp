...
namespace wabby::render::vulkan
{
  ...
  vk::raii::RenderPass build_render_pass( const vk::raii::Device & device, const vk::SurfaceFormatKHR & surface_format )
  {
    ...
    // color attachment
    attachment_descriptions.emplace_back( vk::AttachmentDescription{ .format         = surface_format.format,
    ...
    attachment_references.emplace_back( vk::AttachmentReference{ .attachment = 0, .layout = vk::ImageLayout::eAttachmentOptimal } );

    std::vector<vk::SubpassDescription> subpass_descriptions;
    subpass_descriptions.emplace_back( vk::SubpassDescription{ .pipelineBindPoint    = vk::PipelineBindPoint::eGraphics,
  ...
  void vk_render_pass::begin( const vk::raii::CommandBuffer & buffer,
                              const vk::raii::Framebuffer &   framebuffer,
                              vk::Rect2D                      render_area,
                              vk::ArrayProxy<vk::ClearValue>  clear_values )
  {
    vk::RenderPassBeginInfo render_pass_begin_info{ .renderPass      = *render_pass_,
